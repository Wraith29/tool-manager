...
proc newCommand*(
    name: string, description: string, exec: ExecFn, short: string = ""
): Command =
  return
    Command(name: name, description: description, short: short, kind: ctExe, exec: exec)

proc newCommand*(
    name: string, description: string, children: seq[Command], short: string = ""
): Command =
  return Command(
    name: name,
